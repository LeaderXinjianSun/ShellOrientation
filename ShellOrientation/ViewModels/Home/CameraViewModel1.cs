                    ...
                    case "SaveImage":
                        if (r)
                        {
                            try
                            {
                                ...
                                HOperatorSet.WriteImage(CameraIamge0, "jpeg", 0, System.IO.Path.Combine(System.Environment.CurrentDirectory, filepath, $"1_{DateTime.Now:yyyyMMdd_HHmmss}.jpg"));
                            }
                            catch (Exception ex)
                            {
                    ...
                    if (m806 != null)
                    {
                        if (m806[0] || m806[1])
                        {
                            ...
                            if (img == null)
                            {
                                ...
                                cam.CloseCamera();
                                Thread.Sleep(1000);
                                var r = cam.OpenCamera(param.Camera1Name, "DirectShow");
                                if (r)
                                {
                                    cam.GrabeImageStart();
                                    aggregator.SendMessage("Camera1OpenOK", "Camera");
